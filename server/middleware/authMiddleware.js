...
export const verifyUser = (req, res, next) => {
    ...
    jwt.verify(token, "jwt-secret-key", (err, decoded) => {
        if (err) {
            ...
            console.log('user data:', decoded.user_data)
            //req.user = { user: decoded.user, user_id: decoded.user_id, role: decoded.role, barangay_id: decoded.barangay_id };
            req.user_data = { user: decoded.user_data.user, user_id: decoded.user_data.user_id, role: decoded.user_data.role, profile_image: decoded.user_data.profile_image,
